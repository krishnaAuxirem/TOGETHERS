...
import { ArrowRight, Zap, Shield, Globe, Users, Heart, MessageSquare, Share2 } from 'lucide-react';
    ...
    <section className="py-24 bg-white">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="grid md:grid-cols-3 gap-6">
          {[
            { title: 'Community Matching', desc: 'AI analyzes your interests, skills, and goals to recommend the perfect communities', Icon: Globe, highlight: '94% match accuracy' },
            { title: 'Event Discovery', desc: 'Never miss relevant events with smart notifications based on your calendar and interests', Icon: Zap, highlight: '3x more event attendance' },
            { title: 'Connection Suggestions', desc: 'Meet people who share your ambitions with AI-powered connection recommendations', Icon: Users, highlight: '2M+ connections made' },
          ].map(card => (
            <div key={card.title} className="relative glass-card rounded-3xl p-8 hover-lift group border border-gray-100 overflow-hidden">
              ...
              <div className="relative">
                <div className="w-12 h-12 bg-coral-50 rounded-2xl flex items-center justify-center mb-4">
                  <card.Icon size={22} className="text-coral-500" />
                </div>
          ...
          {posts.map((post, i) => (
            <div key={i} className="glass-card rounded-3xl p-6 hover-lift">
              ...
              <div className="flex items-center gap-6 text-sm text-gray-400 pt-4 border-t border-gray-100">
                <button className="flex items-center gap-1.5 hover:text-coral-500 transition-colors"><Heart size={14} /> {post.likes.toLocaleString()}</button>
                <button className="flex items-center gap-1.5 hover:text-indigo-500 transition-colors"><MessageSquare size={14} /> {post.comments}</button>
                <button className="flex items-center gap-1.5 hover:text-emerald-500 transition-colors"><Share2 size={14} /> Share</button>
              </div>
            </div>
          ))}
        </div>
